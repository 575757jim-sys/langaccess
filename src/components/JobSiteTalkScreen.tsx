...
const SUPERVISOR_PROMPTS = [
  ...
  'Meeting at the job trailer.',
  'Check in with the foreman.',
  'Report any injuries right away.',
  'Watch for overhead hazards.',
  'End of shift — clean your area.',
  'No work without a permit.',
];

interface Message {
  id: number;
  side: 'supervisor' | 'worker';
  original: string;
  translation: string;
}
...
export default function JobSiteTalkScreen({ language, onBack }: JobSiteTalkScreenProps) {
  ...
  const [workerInput, setWorkerInput] = useState<PanelInputState>(freshInput());
  ...
  const sendSupervisor = useCallback(async (text: string) => {
    if (!text.trim()) return;
    setSupervisorInput(s => ({ ...s, busy: true, text: '' }));
    const translation = await doTranslate(text, 'en', langCode);
    addMessage({ side: 'supervisor', original: text, translation });
    setSupervisorInput(s => ({ ...s, busy: false }));
    playAudioFromGesture(translation, language);
  }, [langCode, language]);

  const sendWorker = useCallback(async (text: string) => {
    if (!text.trim()) return;
    setWorkerInput(s => ({ ...s, busy: true, text: '' }));
    const translation = await doTranslate(text, langCode, 'en');
    addMessage({ side: 'worker', original: text, translation });
    setWorkerInput(s => ({ ...s, busy: false }));
  }, [langCode]);

  const startRecording = useCallback(async (side: 'supervisor' | 'worker') => {
    ...
    mr.onstop = async () => {
      ...
      if (blob.size < 500) {
        if (side === 'supervisor') setSupervisorInput(s => ({ ...s, recording: false, busy: false }));
        else setWorkerInput(s => ({ ...s, recording: false, busy: false }));
        return;
      }

      if (side === 'supervisor') setSupervisorInput(s => ({ ...s, recording: false, busy: true }));
      else setWorkerInput(s => ({ ...s, recording: false, busy: true }));

      try {
        const transcript = await doTranscribe(blob, actualMime, side === 'worker' ? language : 'english');
        if (transcript) {
          if (side === 'supervisor') await sendSupervisor(transcript);
          else await sendWorker(transcript);
        } else {
      ...
      if (side === 'supervisor') setSupervisorInput(s => ({ ...s, busy: false }));
      else setWorkerInput(s => ({ ...s, busy: false }));
    };

    mr.start(250);

    if (side === 'supervisor') setSupervisorInput(s => ({ ...s, recording: true }));
    else setWorkerInput(s => ({ ...s, recording: true }));
  }, [language, sendSupervisor, sendWorker]);
  ...
  const clearAll = () => {
    setMessages([]);
    setSupervisorInput(freshInput());
    setWorkerInput(freshInput());
    globalAudio.pause();
    if (mrRef.current && mrRef.current.state !== 'inactive') mrRef.current.stop();
  };

  const replayMessage = (msg: Message) => {
    if (msg.side === 'supervisor') {
  ...
  const workerSendDisabled = !workerInput.text.trim() || workerInput.busy || workerInput.recording;
  const supervisorSendDisabled = !supervisorInput.text.trim() || supervisorInput.busy || supervisorInput.recording;

  return (
    <div className="fixed inset-0 flex flex-col bg-gray-200 overflow-hidden select-none">

      {/* ─── WORKER / CREW panel (top — rotated 180°) ─── */}
      <div className="flex-1 flex flex-col bg-white overflow-hidden rotate-180">
        <div className="flex-shrink-0 flex items-center justify-between px-5 py-3 border-b border-gray-100">
          ...
          {workerInput.busy && <Loader2 className="w-5 h-5 text-gray-400 animate-spin" />}
        </div>

        <div className="flex-1 flex flex-col items-center justify-center px-6 py-4 overflow-hidden">
          {messages.filter(m => m.side === 'supervisor').length > 0 ? (
            (() => {
              const last = messages.filter(m => m.side === 'supervisor').slice(-1)[0];
              return (
            ...
            <p className="text-gray-300 text-center" style={{ fontSize: '18px' }}>
              Supervisor message will appear here
            </p>
          )}
        </div>
        ...
        <div className="flex-shrink-0 border-t border-gray-100 px-5 py-4">
          ...
          <div className="flex items-center gap-3">
            <textarea
              value={workerInput.text}
              onChange={e => setWorkerInput(s => ({ ...s, text: e.target.value }))}
              onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendWorker(workerInput.text); } }}
            ...
            />
            <BigMicBtn
              recording={workerInput.recording}
              busy={workerInput.busy}
              onStart={() => startRecording('worker')}
              onStop={stopRecording}
            />
            <button
              onClick={() => sendWorker(workerInput.text)}
              disabled={workerSendDisabled}
      ...
      {/* ─── Divider / middle bar ─── */}
      <div className="flex-shrink-0 flex items-center bg-orange-50 border-y border-orange-200 px-4 py-2 gap-3 z-10" style={{ minHeight: '48px' }}>
        ...
        <div className="flex-1 flex items-center justify-center gap-2">
          <span className="font-semibold text-orange-700" style={{ fontSize: '13px' }}>Worker or Crew</span>
          <div className="flex items-center gap-1 bg-white border border-orange-300 rounded-full px-2 py-0.5 shadow-sm">
            <ArrowUpDown className="w-3.5 h-3.5 text-orange-500" />
            <span className="font-bold text-orange-500 tracking-widest uppercase" style={{ fontSize: '10px' }}>Job Site Talk</span>
          </div>
          <span className="font-semibold text-gray-700" style={{ fontSize: '13px' }}>Supervisor or Foreman</span>
        </div>
      ...
      {/* ─── SUPERVISOR / FOREMAN panel (bottom — normal orientation) ─── */}
      <div className="flex-1 flex flex-col bg-white overflow-hidden">
        <div className="flex-shrink-0 flex items-center justify-between px-5 py-3 border-b border-gray-100">
          <span className="text-sm font-bold text-gray-400 tracking-widest uppercase">Supervisor or Foreman — English</span>
          <button
        ...
        {showPrompts && (
          <div className="flex-shrink-0 px-4 pt-3 pb-2 overflow-x-auto flex gap-2 no-scrollbar border-b border-gray-100">
            {SUPERVISOR_PROMPTS.map(p => (
              <button
                key={p}
                onClick={() => sendSupervisor(p)}
                className="flex-shrink-0 px-3 py-2 rounded-xl bg-gray-50 border border-gray-200 text-gray-700 hover:border-orange-400 hover:text-orange-700 hover:bg-orange-50 transition-all whitespace-nowrap font-medium"
                style={{ fontSize: '13px' }}
              >
                {p}
              </button>
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6 py-4 overflow-hidden">
          {messages.filter(m => m.side === 'worker').length > 0 ? (
            (() => {
              const last = messages.filter(m => m.side === 'worker').slice(-1)[0];
              return (
                <div className="w-full space-y-2 text-center">
                  <p
                    className="font-bold text-gray-900 leading-snug break-words"
            ...
            <p className="text-gray-300 text-center" style={{ fontSize: '18px' }}>
              Worker or crew responses appear here in English
            </p>
          )}
          {supervisorInput.busy && (
            <div className="flex items-center gap-2 mt-3 text-orange-600" style={{ fontSize: '14px' }}>
        ...
        <div className="flex-shrink-0 border-t border-gray-100 px-5 py-4">
          ...
          <div className="flex items-center gap-3">
            <textarea
              value={supervisorInput.text}
              onChange={e => setSupervisorInput(s => ({ ...s, text: e.target.value }))}
              onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendSupervisor(supervisorInput.text); } }}
              placeholder="Type or speak in English…"
              rows={1}
              className="flex-1 bg-gray-50 border border-gray-200 text-gray-900 placeholder-gray-300 rounded-2xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-orange-500 resize-none"
              style={{ fontSize: '18px' }}
            />
            <BigMicBtn
              recording={supervisorInput.recording}
              busy={supervisorInput.busy}
              onStart={() => startRecording('supervisor')}
              onStop={stopRecording}
            />
            <button
              onClick={() => sendSupervisor(supervisorInput.text)}
              disabled={supervisorSendDisabled}
              className="p-3 rounded-2xl bg-orange-600 hover:bg-orange-500 disabled:opacity-25 disabled:cursor-not-allowed text-white transition-colors flex-shrink-0"
            >
              <Send className="w-5 h-5" />
            </button>
          </div>
